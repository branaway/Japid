...
Starting from version 0.7.1, the @for@ has an "open" form that gives template authors access to a few implicit variables related to the loop</code></pre>The variables are explained in the following section, but here is the syntax, basically you leave out the "()" and the "{}"
...
* Since this form involves multiple method dispatching, it's slower than the raw @for@ loop. But the difference is in microsecond scale and probably does not matter. 
