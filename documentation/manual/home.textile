...
# 2011/2/20, added a section for the enhanced @if@ statement 
# 2011/2/22, edited; added "open for" 
...
The caller side does not need to provides arguments for all the parameters. The missing ones default to null.  

Note: you cannot mix these two conventions. You either use arguments without any name prefix or all the arguments must the named format. 
...
New feature since 0.8.9: sometimes authors may want to store the doBody result in a local String variable. One can acheive that using a method named @renderBody()@. E.g.:
...
h2. Use Japid in Modules (version 0.8.7 and up)
